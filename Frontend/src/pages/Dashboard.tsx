...
import { Pencil, Trash2, FileDown, LogOut, Save, Moon, Sun } from "lucide-react";
...
import logoDark from "@/assets/logo-texto-branco.png"; // Texto branco
import logoLight from "@/assets/logo-texto-preto.png"; // Texto preto
...
const Dashboard = () => {
  ...
  const [mounted, setLoadingMounted] = useState(false);
  ...
  return (
    <main className={cn(
      "relative min-h-screen w-full text-foreground transition-colors duration-300",
      isDark ? "bg-background" : "bg-zinc-100"
    )}>
      {/* Background Fixo (Garante que o degradê cubra tudo ao rolar) */}
      <div
        aria-hidden
        className="pointer-events-none fixed inset-0 z-0"
        style={{
          background: isDark
            ? "radial-gradient(ellipse 80% 60% at 50% -10%, hsl(var(--primary) / 0.25), transparent 70%), radial-gradient(ellipse 60% 50% at 50% 110%, hsl(var(--primary) / 0.15), transparent 70%)"
            : "radial-gradient(ellipse 80% 60% at 50% -10%, hsl(var(--primary) / 0.22), transparent 70%), radial-gradient(ellipse 60% 50% at 50% 110%, hsl(var(--primary) / 0.12), transparent 70%)",
        }}
      />
      
      <header className={cn(
        "relative z-20 border-b backdrop-blur transition-colors duration-300",
        isDark ? "border-border/60 bg-card/60" : "border-zinc-200 bg-white/70"
      )}>
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-foreground")}>
                Olá, {user.nome_real || user.email.split('@')[0]}
              </p>
              <p className={cn("text-xs", isDark ? "text-zinc-200" : "text-muted-foreground")}>
                {user.tipo_usuario || "Membro"} · {user.igreja || "Portugal"}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Sun className={cn("h-4 w-4", isDark ? "text-zinc-500" : "text-yellow-500")} />
              <Switch 
                checked={isDark} 
                onCheckedChange={(checked) => setTheme(checked ? "dark" : "light")}
              />
              <Moon className={cn("h-4 w-4", isDark ? "text-primary" : "text-zinc-500")} />
            </div>
            ...
            >
              {(["Materna", "Paterna", "Não aplicável"] as Lineage[]).map((opt) => (
                <Label
                  ...
                  htmlFor={`lineage-${opt}`}
                  className={cn(
                    "flex cursor-pointer items-center gap-2.5 rounded-lg border bg-background px-3.5 py-2.5 text-sm font-normal transition-smooth hover:border-primary/40",
                    lineage === opt
