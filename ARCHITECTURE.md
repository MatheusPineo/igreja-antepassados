...
- **Frontend Estrito (Vercel)**: A Vercel atua no projeto de modo enjaulado, operando estritamente dentro da pasta `Frontend` (onde o Root Directory está configurado para `Frontend` no painel da Vercel). Configurações restritas do arquivo [Frontend/vercel.json](file:///c:/Users/mathe/projeto_messianica/Frontend/vercel.json) utilizam a propriedade `"routes"` com o manipulador `"handle": "filesystem"` seguido de um fallback de roteamento SPA para `/index.html`. Isso garante que todos os arquivos estáticos e assets sejam servidos perfeitamente e que atualizações manuais de página (Ctrl + F5) em rotas cliente como `/dashboard` não disparem erro 404, enquanto mantém os cabeçalhos `Cross-Origin-Opener-Policy: same-origin-allow-popups` exigidos pelo Google Login. É vetada a interpretação Serverless ou a presença de APIs no frontend para impedir interceptação indesejada de fluxos Python.
- **Backend Limitado (Render)**: Limitado severamente a 512MB RAM em sua camada gratuita.
  ...
  - *Mitigação de OOM (Out Of Memory na Exportação)*: Geração de Relatórios (PDF) utilizando buffers lógicos matemáticos (`io.BytesIO`) através da biblioteca ReportLab. O layout migrou de coordenadas físicas estáticas para uma arquitetura baseada em `SimpleDocTemplate` e `Flowables` (`Paragraph`, `Spacer` e `Table`), garantindo o manuseio automático de quebras de página para listagens extensas (64+ registros) e evitando qualquer tipo de overflow de texto. Baniu-se o uso de motores pesados baseados em Headless Webkit/Chromium ou geradores HTML2PDF nativos.
